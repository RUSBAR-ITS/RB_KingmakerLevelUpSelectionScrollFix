...
namespace KingmakerPartySelectionExpansion
{
    ...
    internal static class PartySelectionUiScrollPatch
    {
        private const string WrapperName = "KingmakerPartySelectionExpansion_ActivePartyScrollRect";
        private const string ViewportName = "KingmakerPartySelectionExpansion_ActivePartyViewport";
        private const string ScrollbarName = "KingmakerPartySelectionExpansion_ActivePartyScrollbar";

        private static int s_postfixRunCount;
        private static int s_dumpRunCount;
        private static int s_diagnosticLogCount;

        private static void Postfix(GroupManager __instance)
        {
            if (Main.Settings == null || !Main.Settings.EnablePatch || !Main.Settings.EnableActivePartyScroll)
            {
                return;
            }
            ...
            {
                Logger.Exception("GroupManager.Fill postfix failed.", ex);
            }
        }
        ...
        private static void Apply(GroupManager instance)
        {
            if (instance == null)
            {
                Logger.Warning("GroupManager.Fill postfix received a null instance.");
                return;
            }

            RectTransform activeContent = GetFieldValue<Transform>(instance, "ActiveCompanions") as RectTransform;
            if (activeContent == null)
            {
                Logger.Warning("ActiveCompanions is missing or is not a RectTransform; cannot add active party scrolling.");
                return;
            }

            RectTransform remoteContent = GetFieldValue<Transform>(instance, "RemoteCompanions") as RectTransform;

            if (ShouldLogDiagnostic())
            {
                Logger.Info(
                    "GroupManager.Fill postfix run "
                    + s_postfixRunCount
                    + ". ActiveCompanions="
                    + GetPath(activeContent)
                    + ", childCount="
                    + activeContent.childCount
                    + ", maxActivePartySize="
                    + PartySelectionLimit.GetMaxActivePartySize()
                    + ".");
            }

            PartySelectionScrollMarker marker = activeContent.GetComponent<PartySelectionScrollMarker>();
            if (marker == null || !marker.IsValid)
            {
                marker = WrapActiveCompanions(activeContent, remoteContent, marker);
            }

            if (marker == null || !marker.IsValid)
            {
                Logger.Warning("Could not create a valid scroll wrapper for ActiveCompanions.");
                return;
            }

            UpdateWrapper(marker, activeContent, remoteContent);
            DumpDiagnostics(instance, activeContent, remoteContent, marker);
        }

        private static PartySelectionScrollMarker WrapActiveCompanions(
            RectTransform content,
            RectTransform remoteContent,
            PartySelectionScrollMarker existingMarker)
        {
            Transform originalParent = content.parent;
            if (originalParent == null)
            {
                Logger.Warning("ActiveCompanions has no parent; cannot wrap it.");
                return null;
            }

            int siblingIndex = content.GetSiblingIndex();
            RectTransformSnapshot contentSnapshot = RectTransformSnapshot.Capture(content);
            float originalHeight = Mathf.Abs(content.rect.height) > 1f ? Mathf.Abs(content.rect.height) : content.sizeDelta.y;
            ...
            CopyLayout(sourceLayoutElement, wrapperLayoutElement);

            GameObject viewportObject = new GameObject(ViewportName, typeof(RectTransform), typeof(RectMask2D), typeof(Image));
            ...
            string templatePath;
            Scrollbar verticalScrollbar = CreateVerticalScrollbar(wrapper, content, remoteContent, out templateApplied, out templatePath);
            ...
            PartySelectionScrollMarker marker = existingMarker ?? content.gameObject.AddComponent<PartySelectionScrollMarker>();
            ...
            marker.OriginalHeight = Mathf.Max(1f, originalHeight);
            marker.HasKnownScrollPosition = false;

            Logger.Info(
                "Wrapped ActiveCompanions once. Parent="
                + GetPath(originalParent)
                + ", wrapper="
                + GetPath(wrapper)
                + ", content="
                + GetPath(content)
                + ", originalHeight="
                + marker.OriginalHeight.ToString("0.0")
                + ", templateScrollbar="
                + (templateApplied ? templatePath : "<fallback>")
                + ".");
        ...
        private static void UpdateWrapper(
            PartySelectionScrollMarker marker,
            RectTransform content,
            RectTransform remoteContent)
        {
            ...
            TryApplyTemplateScrollbar(marker, content, remoteContent);

            if (marker.VerticalScrollbar != null)
            {
                bool showScrollbar = Main.Settings.ShowActivePartyScrollbar;
            ...
            float viewportHeight = GetViewportHeight(marker);
                    ...
                    || marker.LastLoggedChildCount != content.childCount))
            {
                ...
                Logger.Info(
                    "Active party scroll wrapper updated. contentHeight="
                    + contentHeight.ToString("0.0")
                    + ", viewportHeight="
                    + viewportHeight.ToString("0.0")
                    + ", childCount="
                    + content.childCount
                    + ", activeChildCount="
                    + CountActiveChildren(content)
                    + ", scrollbar="
                    + (Main.Settings.ShowActivePartyScrollbar ? "enabled" : "disabled")
                    + ", verticalPosition="
                    + verticalPosition.ToString("0.000")
                    + ".");
            }
        }

        private static float GetViewportHeight(PartySelectionScrollMarker marker)
        {
            float maxHeight = Mathf.Max(120f, Main.Settings.MaxActivePartyListHeight);
            if (marker.OriginalHeight > 1f)
            {
                return Mathf.Clamp(marker.OriginalHeight, 120f, maxHeight);
            }
        ...
        {
            Scrollbar template = FindTemplateScrollbar(content, remoteContent);
            ...
            if (clonedScrollbar != null)
            {
                ...
                Logger.Info("Cloned party selection scrollbar style from " + templatePath);
        ...
        private static void TryApplyTemplateScrollbar(
            PartySelectionScrollMarker marker,
            RectTransform content,
            RectTransform remoteContent)
        {
            ...
            Logger.Info("Replaced fallback scrollbar with cloned party selection scrollbar style from " + marker.TemplateScrollbarPath);
        }

        private static Scrollbar FindTemplateScrollbar(RectTransform activeContent, RectTransform remoteContent)
        {
            ScrollRect remoteScrollRect = FindOwningScrollRect(remoteContent);
            if (remoteScrollRect != null && remoteScrollRect.verticalScrollbar != null)
            {
                return remoteScrollRect.verticalScrollbar;
            }

            Transform root = FindDiagnosticRoot(activeContent);
            if (root == null)
            {
                return null;
            }
        ...
        private static ScrollRect FindOwningScrollRect(RectTransform content)
        {
            if (content == null)
            {
                return null;
            ...
            while (current != null)
            {
                ScrollRect[] scrollRects = current.GetComponents<ScrollRect>();
                for (int i = 0; i < scrollRects.Length; i++)
                {
                    ScrollRect scrollRect = scrollRects[i];
                    if (scrollRect != null && scrollRect.content == content)
                    {
                        return scrollRect;
                    }
                }
        ...
        private static Scrollbar CloneScrollbarFromTemplate(Scrollbar template, RectTransform wrapper)
        {
            ...
            float width = templateRect != null ? Mathf.Max(12f, templateRect.rect.width) : 18f;
        ...
        private static Scrollbar CreateFallbackVerticalScrollbar(RectTransform wrapper)
        {
            ...
            RectTransform scrollbarRect = scrollbarObject.GetComponent<RectTransform>();
            scrollbarRect.SetParent(wrapper, false);
            ConfigureScrollbarRect(scrollbarRect, 14f);
        ...
        private static void CopyLayout(LayoutElement source, LayoutElement target)
        {
        ...
        private static T GetFieldValue<T>(object instance, string fieldName) where T : class
        {
            ...
            FieldInfo field = AccessTools.Field(instance.GetType(), fieldName)
                ?? AccessTools.Field(typeof(GroupManagerBase), fieldName);
            return field != null ? field.GetValue(instance) as T : null;
        }

        private static bool ShouldLogDiagnostic()
        {
        ...
        private static void DumpDiagnostics(
            ...
            RectTransform activeContent,
            RectTransform remoteContent,
            PartySelectionScrollMarker marker)
        {
            if (!Main.Settings.DumpGroupManagerUiHierarchy)
            {
                return;
            }
            ...
            Transform root = FindDiagnosticRoot(instance != null ? instance.transform : activeContent);
            int maxDepth = Mathf.Clamp(Main.Settings.DumpUiHierarchyMaxDepth, 1, 40);
            int maxNodes = Mathf.Clamp(Main.Settings.DumpUiHierarchyMaxNodes, 50, 20000);

            StringBuilder builder = new StringBuilder(64 * 1024);
            builder.AppendLine("Group manager UI dump " + s_dumpRunCount + "/" + maxRuns + ".");
            builder.AppendLine("Root=" + GetPath(root) + ", maxDepth=" + maxDepth + ", maxNodes=" + maxNodes);
            builder.AppendLine("ActiveCompanions=" + GetPath(activeContent) + ", childCount=" + activeContent.childCount);
            builder.AppendLine("RemoteCompanions=" + GetPath(remoteContent));
        ...
        private static Transform FindDiagnosticRoot(Transform transform)
        {
            ...
            {
                last = current;
                if (current.name.IndexOf("Group", StringComparison.OrdinalIgnoreCase) >= 0
                    || current.name.IndexOf("Party", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    return current;
                }
        ...
        private static void AppendScrollComponentSummary(StringBuilder builder, Transform root)
        {
            ...
            for (int i = 0; i < scrollRects.Length; i++)
            {
                ...
                builder.AppendLine(
                    ...
                    + " content="
                    + GetPath(scrollRect.content)
                    + " viewport="
                    + GetPath(scrollRect.viewport)
                    + " vScrollbar="
                    + GetPath(scrollRect.verticalScrollbar != null ? scrollRect.verticalScrollbar.transform : null));
            }
            ...
            for (int i = 0; i < scrollbars.Length; i++)
            {
                ...
                builder.AppendLine(
                    "  [" + i + "] "
                    + GetPath(scrollbar.transform)
                    + " active="
                    + scrollbar.gameObject.activeInHierarchy
                    + " direction="
                    + scrollbar.direction
                    + " handle="
                    + GetPath(scrollbar.handleRect)
                    + rectInfo);
            }
        }
    ...
    public sealed class PartySelectionScrollMarker : MonoBehaviour
    {
