...
namespace KingmakerCategorizedFeatMultiplier
{
    internal static class ModLocalization
    {
        ...
        private static readonly HashSet<string> s_SupportedLocales = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "enGB",
            "ruRU"
        };
        ...
        private static string s_CurrentLocaleCode;
        ...
        {
            get { return string.IsNullOrEmpty(s_CurrentLocaleCode) ? FallbackLocale : s_CurrentLocaleCode; }
        }
        ...
        internal static void Reload()
        {
            ...
            s_CurrentLocaleCode = localeCode;
            s_CurrentStrings = localeCode == FallbackLocale ? s_FallbackStrings : LoadLocale(localeCode);
            Logger.Info("Localization loaded. locale=" + CurrentLocaleCode);
        }
        ...
        private static string ResolveLocaleCode()
        {
            ...
            ModLanguage language = settings != null ? settings.Language : ModLanguage.Auto;
            switch (language)
            {
                case ModLanguage.English:
                    return "enGB";
                case ModLanguage.Russian:
                    return "ruRU";
                default:
                    return DetectGameLocaleCode();
            }
        }

        private static string DetectGameLocaleCode()
        {
            try
            {
                string locale = NormalizeLocaleCode(LocalizationManager.CurrentLocale.ToString());
            ...
            {
                Logger.Warning("Could not read game locale; using fallback. " + ex.Message);
            }

            string culture = CultureInfo.CurrentUICulture.Name;
            return culture.StartsWith("ru", StringComparison.OrdinalIgnoreCase) ? "ruRU" : FallbackLocale;
        }
        ...
        private static string NormalizeLocaleCode(string locale)
        {
            ...
            locale = locale.Trim();
            if (s_SupportedLocales.Contains(locale))
            {
                return locale;
            }

            if (locale.StartsWith("ru", StringComparison.OrdinalIgnoreCase)
                || locale.IndexOf("Russian", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                return "ruRU";
            }

            if (locale.StartsWith("en", StringComparison.OrdinalIgnoreCase)
                || locale.IndexOf("English", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                return FallbackLocale;
            }

            Logger.Warning("Unsupported game locale value '" + locale + "'; using culture/fallback locale.");
            return string.Empty;
        }
